...
case class Conditional(conclusion: Fact, conditions: Seq[Condition]) extends Statement {
  def propagate(facts: Map[Class[_], Set[Fact]], moves: Map[Role, Action], state: Option[GameState]): Map[Class[_], Set[Fact]] = {
    ...
    val newFacts = values.toSet.map(conclusion.substitute).groupBy(f => f.getClass).toMap
  ...
  def proves(fact: Fact, facts: Map[Class[_], Set[Fact]], moves: Map[Role, Action], state: Option[GameState]) = {
